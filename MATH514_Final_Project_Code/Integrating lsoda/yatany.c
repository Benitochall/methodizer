...
int getExactSolution(void){
        ...
        y[0] = 0.02; // the starting values of each paramter
		      // TODO need to calc the starting value of tanx
...
for(int i =1; i<=5000; i++){
        ...
        y_init = (double)y_init + 0.001*atan(y_init); 
	total_fxn_evals+=1; 
...
for(int i =1; i<=5000; i++){
	...
	fxn_evals +=4; 
        double k1 = atan(y_init);
        double k2 = atan(y_init + 0.5*stepsize*k1);  
        double k3 = atan(y_init + 0.5 * stepsize *k2); 
        double k4 = atan(y_init + 1.0* stepsize *k3); 
...
for(int i =1; i<=5000; i++){
        ...
        double k2 = atan(y_init + (0.5*stepsize*k1));
        double k3 = atan(y_init + (0.5 * stepsize *k2));
        double k4 = atan(y_init +( stepsize *k3)); 
...
for (int i=4; i<=5000; i++){
        ...
        y_init= y_3 + (double) 1 /24.0 * stepsize * (55 *atan( y_3) + -59 *atan( y_2) + 37 *atan(y_1) -9 *atan(y_0));
	total_fxn_evals +=4; 
        // update values
                y_0 = y_1; 
                y_1= y_2; 
...
//	double v_exact = exactArray[i]; 
//	fprintf(output_file, "%0.3f %0.15f %0.15f %0.15f\n", v, y_init, v_exact, y_init -v_exact);
...
for (int i=4; i<=5000; i++){
        ...
        y_init= y_3+ 1.0 /24.0 * stepsize * (55.0 * atan(y_3) + -59.0 *atan( y_2) + 37.0 *atan(y_1) -9.0 *atan(y_0));
...
for(int i =1; i<=5000; i++){
        ...
        double k1 =atan( y_init);
        double k2 = atan(y_init + stepsize *2*(k1/27.0)); 
        double k3 = atan( y_init  +  stepsize /((k1/36.0 + k2/12.0))); 

        double k4 =atan( y_init + stepsize * (k1/24.0 + k3/8.0)); 
        double k5 = atan(y_init + stepsize *(5.0/12.0*k1 + -25.0/16.0*k3+  25.0/16.0*k4));
        double k6 = atan(y_init  + stepsize * (k1/20.0 + k4/4.0 + k5/5.0)); 
        double k7 = atan(y_init + stepsize *(-25.0/108.0*k1 + 125.0/108.0*k4 - 65.0/27.0*k5 + 125.0/54.0*k6));

        double k8 = atan(y_init + stepsize * (31.0/300.0*k1 + 61.0/225.0*k5 - 2.0/9.0*k6 + 13.0/900.0*k7));
        double k9 = atan(y_init  + stepsize *(2.0*k1 + -53.0/6.0*k4 + 704.0/45.0*k5 + -107.0/9.0*k6 + 67.0/90.0*k7 + 3.0*k8));
        double k10 = atan( y_init + stepsize *(-91/108*k1 + 23/108*k4 + -976/135*k5 + 311/54.0*k6 + -19.0/60.0*k7 + 17.0/6.0*k8 - k9/12.0));
 ...
 //       double k11 = y_init + stepsize*(2383.0/4100.0*k1 + -341.0/164.0*k4 + 4496.0/1025.0*k5 + -301.0/82.0*k6 + 2133.0/4100.0*k7 + 45.0/82.0*k8 + 45.0/164.0*k9 + 18.0/41.0*k10); 
        double k12 = atan(y_init  + stepsize * (3.0/205.0*k1 + -6.0/41.0*k6 + -3.0/205.0*k7 + -3.0/41.0*k8 + 3.0/41.0*k9 + 6.0/41.0*k10));
        double k13 = atan(y_init + stepsize *(-1777.0/4100.0*k1 + -341.0/164.0*k4 + 4496.0/1025.0*k5 + -289.0/82.0*k6 + 2193.0/4100.0*k7 + 51.0/82.0*k8 + 33.0/164.0*k9 + 12.0/41.0*k10 + k12)); 
...
for(int i =1; i<=5000; i++){
        ...
        double k8 = atan(y_init + stepsize * (31.0/300.0*k1 + 61.0/225.0*k5 - 2.0/9.0*k6 + 13.0/900.0*k7));
        double k9 = atan(y_init  + stepsize *(2.0*k1 + -53.0/6.0*k4 + 704.0/45.0*k5 + -107.0/9.0*k6 + 67.0/90.0*k7 + 3.0*k8));
        double k10 = atan( y_init + stepsize *(-91.0/108.0*k1 + 23.0/108.0*k4 + -976.0/135.0*k5 + 311.0/54.0*k6 + -19.0/60.0*k7 + 17.0/6.0*k8 - k9/12.0));
...
FILE *output_file = fopen("atany_out.txt", "w"); // write only
